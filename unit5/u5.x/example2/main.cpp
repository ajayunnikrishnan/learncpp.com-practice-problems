...
{
	constexpr double gravity{ 9.8 }; // added constexpr

	// Using formula: [ s = u * t + (a * t^2) / 2 ], here u(initial velocity) = 0
	const double distanceFallen{ (gravity * (seconds * seconds)) / 2.0 }; // added runtime constant
	const double currentHeight{ towerHeight - distanceFallen };  // added runtime constant
...
int main()
{
	const double towerHeight{ getTowerHeight() }; // added const
